...
{
    uint32_t R;
    R=*Rm+((~*Rn)+1); /* Suma los valores de las direcciones Rm y Rn */
...
{
	*Rn=*Rm>>32;  /* Desplaza a la derecha 32 bit  el valor en la dirección Rm */
	*Rn=*Rn<<32; /* Desplaza a la izquierda 16 bits el valor en la dirección Rm */
...
void ASRS(uint32_t *Rm,uint32_t *Rn) /* Función que no retorna, pero realiza un desplazamiento aritmético a la derecha */
{
	...
	*Rm=(*Rm>>1)|*Rn; /* Realiza la operción OR entre las direcciones Rm y Rn, y el resultado se guarda en la dirección Rm */
	comp=1; /* No modifica una bandera */
	flag(Rm,Rm,Rn,banderas,&comp); /* Se dirije a la función flag para determinar si en el resultado hay banderas */
...
}/* Función que no retorna, pero realiza una rotación de los datos */
