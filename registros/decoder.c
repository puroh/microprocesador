...
void decodeInstruction(instruction_t instruction)
{
	...
	if( strcmp(instruction.mnemonic,"POP") == 0 )
	{
        ...
        {
			bin+=(instruction.registers_list[i]<<i);
            mvprintw(15,40,"Instruccion binaria :%.8X ",bin); //Imprime en interfaz mediante curses
        }
		bin=(11<<12)+(3<<10)+(instruction.registers_list[15]<<8);
        mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
	    mvprintw(10,40,"instruccion :%s ",instruction.mnemonic); //Imprime en interfaz mediante curses
        POP(registers,memoria,instruction.registers_list); //Imprime en interfaz mediante curses
        //mostrar_memoria(memoria,MEMORIA)
	}
if( strcmp(instruction.mnemonic,"ADD") == 0 ) // compara el mnemonico con ADD
	{
	    ...
	    mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
	    mvprintw(5,40,"Instruccion :%s %c%d,%c%d,%c%d",instruction.mnemonic,instruction.op1_type,instruction.op1_value,instruction.op2_type,instruction.op2_value,instruction.op3_type,instruction.op3_value); //Imprime en interfaz mediante curses
	...
	if( strcmp(instruction.mnemonic,"MULS") == 0 ){
        ...
        mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
	    mvprintw(5,40,"instruccion :%s %c%d,%c%d,%c%d\n",instruction.mnemonic,instruction.op1_type,instruction.op1_value,instruction.op2_type,instruction.op2_value,instruction.op3_type,instruction.op3_value); //Imprime en interfaz mediante curses
	...
	if( strcmp(instruction.mnemonic,"MOV") == 0 ) //compara el mnemonico con MOV
	{
	    ...
	    mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
	    mvprintw(5,40,"Instruccion :%s %c%d,%c%d",instruction.mnemonic,instruction.op1_type,instruction.op1_value,instruction.op2_type,instruction.op2_value); //Imprime en interfaz mediante curses
	...
	if( strcmp(instruction.mnemonic,"CMP") == 0 ) //compara el mnemonico con CMP
	{
	    ...
	    mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
        mvprintw(5,40,"Instruccion :%s %c%d,%c%d\n",instruction.mnemonic,instruction.op1_type,instruction.op1_value,instruction.op2_type,instruction.op2_value); //Imprime en interfaz mediante curses
	...
	if( strcmp(instruction.mnemonic,"LDR") ==0)
    {
        ...
        if((instruction.op1_type=='R') && (instruction.op2_type=='P') && (instruction.op3_type=='#')) 
        {
    ...
    if( strcmp(instruction.mnemonic,"LDRH") ==0) //Realiza la funcion de carga LDRH
    {
        if((instruction.op1_type=='R') && (instruction.op2_type=='R') && (instruction.op3_type=='#')) //Condicion para reconocer la instruccion
        {
	...
	if(strncmp(instruction.mnemonic,"B",1)==0) //se compara  con strncmp para comparar la primer letra del mnemonico con B para no aumentar pc si se ejecuta un B
	{
        ...
        if( strcmp(instruction.mnemonic,"BEQ") == 0 ) //compara BEQ con el mnemonico de las instrucciones en el .txt
		{
		    ...
		    mvprintw(15,40,"Instruccion binaria :%.8X \n",bin); //Imprime en interfaz mediante curses
            mvprintw(5,40,"Instruccion :%s %c%d",instruction.mnemonic,instruction.op1_type,instruction.op1_value); //Imprime en interfaz mediante curses
...
void obtener_registros(uint32_t *pcount) //funcion que obtiene el valor en cada reguistro
{
    uint32_t i; //declaracion de variable
    for(i=0;i<16;i++) //condicion para obtener los registros
	{
		pcount[i]=registers[i]; //Igualacion del contador en mameoria con cada registro
	}
}
void obtener_memoria(uint32_t *pcount) //Obtiene los valores en memoria
{
	uint8_t i; //declaracion de variables
    for(i=0;i<MEMORIA;i++) //condicion para ir obteniendo valor por valor
	{
		pcount[i]=memoria[i]; //igualacion con cada valor almacenado en la ram para obtener los valores en memoria
	}
}
instruction_t getInstruction(char* instStr)  //Funcion que permite obtener la instruccion
{
